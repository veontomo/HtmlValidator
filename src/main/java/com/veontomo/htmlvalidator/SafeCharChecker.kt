...
class SafeCharChecker : Checker() {
    override val descriptor = "Safe char checker"
    ...
    override fun check(html: String): List<CheckMessage> {
        ...
        lines.forEachIndexed { i, s ->
            ...
            if (!res.isEmpty()) {
                messages.add(CheckMessage("line ${i + 1} \"$s\" contains non-ascii symbols: ${res.joinToString { it -> "\"$it\" (ascii code: ${it.toInt()})" }}", false, descriptor))
            }
        }
        return messages
    }
